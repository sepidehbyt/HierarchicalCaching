...
mean_size = cfg.snm_mean_content_size
arrival_rate = cfg.snm_new_arrival_rate
...
def generate_snm_requests(sim_days, number_of_bs):
    # (ts, f, size, bs)
    requests = []
    add_index = 0
    for i in range(number_of_bs):
        ...
        for j in range(number_of_requests):
            ...
            volume = volumes[j]
            request = generate_fix_rate(snm_lifespan, volume / snm_lifespan, volume)
